      ...
      <div class="flex items-center justify-center h-full">
        ...
        <n-form ref="formRef" :model="form" :rules="rules" @submit.prevent="onSubmit">
          ...
          <n-form-item path="password">
            <n-input v-model:value="form.password" :type="showPassword ? 'text' : 'password'" :placeholder="t('login.password')">
              <template #prefix>
                <n-icon>
                  <LockClosedOutline />
                </n-icon>
        ...
        <div class="text-center text-sm mt-2">
          {{ t('login.noAccount') }}
          <a class="text-indigo-600 dark:text-indigo-400 hover:underline cursor-pointer" @click="router.push('/register-org')">
